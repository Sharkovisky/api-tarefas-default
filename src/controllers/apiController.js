...
exports.login = (req, res) => {
    ...
    conexao.query(query, [email], (err, rows) => {
        if(err){
            res.status(500)
            res.json({"auth": false, "message": "Erro interno do Servidor"})
            console.log(err)
            ...
            bcrypt.compare(senha, rows[0].senha, (err, resp) => {
                if(resp){
                    ...
                    res.status(403)
                    res.json({"auth": false, "message": "E-mail ou senha incorreto2s"})
                }
            })
        } else {
            res.status(403)
            res.json({"auth": false, "message": "E-mail ou senha incorreto1s"})
        }
    })
}
...
exports.verifica = (req, res, next) => {
    ...
    if(!token){
        res.status(401)
        res.json({"auth": false, "message": "Token em branco"})
    } else {
